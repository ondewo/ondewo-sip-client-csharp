...
Almost everything in this repository is generated. `api/` and `Ondewo.SIP.Client.csproj` are the
output of the `ondewo-csharp-proto-compiler` docker image running over the `ondewo-sip-api` submodule, and they are
overwritten in full by `make build`.

- A change to the **API surface** (a message, a field, an RPC) belongs in
  [ondewo-sip-api](https://github.com/ondewo/ondewo-sip-api), not here. Once it is released there, bump
  `ONDEWO_SIP_API_GIT_BRANCH` in the `Makefile` and run `make build`.
