<div align="center">
  ...
  <h1 align="center">
    ONDEWO SIP Client C-Sharp
  </h1>
</div>

## Overview

`Ondewo.SIP.Client` is a compiled version of the
[ONDEWO SIP API](https://github.com/ondewo/ondewo-sip-api) — the gRPC interface to ONDEWO
SIP Telephony — generated with the
...
over the `ondewo-sip-api` submodule, and it is regenerated in full by `make build`.

## Installation

The package is published to [nuget.org](https://www.nuget.org/packages/Ondewo.SIP.Client) as
**`Ondewo.SIP.Client`**. No custom feed, credential or `nuget.config` entry is needed — the default
`nuget.org` source is enough.

With the .NET CLI, from the directory of the project that should consume it:

```shell
dotnet add package Ondewo.SIP.Client
```
...
```shell
git clone https://github.com/ondewo/ondewo-sip-client-csharp.git   ## Clone the repository
cd ondewo-sip-client-csharp                                        ## Change into the repo directory
...
// `package ondewo.sip;` becomes `Ondewo.<Pascal>` — check the `namespace` line at the top of
...
├── ondewo-sip-api                               <----- submodule: the .proto sources
├── ondewo-proto-compiler                    <----- submodule: the code generator
├── Ondewo.SIP.Client.csproj     <----- generated project file (tracked)
...
| `generate_ondewo_protos`              | runs that image over `ondewo-sip-api/ondewo` and writes the library back into the repo  |
...
- The generated `Ondewo.SIP.Client.csproj` is **tracked**. On the next run the image finds it in the
...
The same glob is why `Ondewo.SIP.Client.csproj` carries
...
Bump `ONDEWO_SIP_VERSION` in the `Makefile`, add a `RELEASE.md` entry in the existing format under a
`## Release ONDEWO SIP Csharp Client <version>` heading — `build_gh_release` slices the release notes out by
...
Pushing a version tag (`5.4.0`, `5.4.0-rc.1` — the shape `make create_release_tag` writes) also triggers
